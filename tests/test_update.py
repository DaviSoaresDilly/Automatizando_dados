...
from application.base_de_dados.update_data import (
    update_atendimento_status,
    update_paciente_info,
)
...
def test_update_atendimento_status(session):
    ...
    updated_atendimento = (
        session.query(Atendimento).filter_by(id=atendimento.id).first()
    )
...
def test_update_paciente_info(session):
    ...
    assert (
        update_paciente_info(session, paciente.id, nome="Teste Paciente", idade=30)
        == True
    )
