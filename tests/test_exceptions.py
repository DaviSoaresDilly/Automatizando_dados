...
def test_integrity_error(session):
    ...
    invalid_atendimento = Atendimento(
        id_paciente=9999, id_bairro=1, id_doenca=1, id_clinica=1
    )
...
def test_database_disconnection(session):
    ...
    with pytest.raises(Exception):
        session.query(
            Paciente
        ).first()  # Tenta acessar o banco de dados após desconexão
...
def test_invalid_data_insertion(session):
    ...
    assert "NOT NULL constraint failed" in str(
        e_info.value
    ), "Erro não esperado: violação da restrição NOT NULL"
