...
def validar_dados_atendimento(data):
    ...
    for campo in campos_obrigatorios:
        if campo not in data:
            raise ValueError(
                f"Campo obrigatório '{campo}' não está presente nos dados."
            )
    ...
    if data.get("Clinica").get("capacidade_diaria") <= 0:
        raise ValueError(
            f"A clínica {data.get('Clinica').get('nome')} não tem capacidade para atendimentos."
        )

    # Verificar se a doença requer cirurgia e se a clínica tem leitos
    if (
        data.get("Doenca").get("requer_cirurgia")
        and data.get("Clinica").get("capacidade_leito") <= 0
    ):
        raise ValueError(
            f"A clínica {data.get('Clinica').get('nome')} não possui leitos para cirurgias."
        )
    ...
    if not isinstance(data["HoraAtendimento"], str):  # Exemplo de verificação
        raise ValueError(
            "O campo 'HoraAtendimento' deve ser uma string no formato HH:MM:SS."
        )
    ...
    if data.get("status") not in status_validos:
        raise ValueError(
            f"Status '{data.get('status')}' é inválido. Valores permitidos: {status_validos}"
        )

    # Validação de médico especializado para doenças graves
    if data.get("Doenca").get("gravidade") in ["Grave", "Muito Grave"]:
        if data.get("Medico").get("especialidade") != data.get("Doenca").get(
            "especialista"
        ):
            raise ValueError(
                f"O médico selecionado não é especializado para tratar a doença '{data.get('Doenca').get('nome')}'."
            )
