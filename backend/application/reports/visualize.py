...
def plot_ocupacao_historico_previsao(df_historico, df_previsao, clinica_nome, fig, ax):
    ax.plot(df_historico["data"], df_historico["Ocupação Histórica"], label="Histórico de Ocupação", color="blue", marker="o")
    ax.plot(df_previsao["data"], df_previsao["Ocupação Prevista"], label="Previsão de Ocupação", color="red", linestyle="--", marker="x")
...
def plot_distribution_by_age_gender(df_distribuicao, fig, ax):
    sns.barplot(data=df_distribuicao, x='Faixa Etária', y='Incidências', hue='Sexo', errorbar=None, ax=ax)  # Usar `errorbar=None` ao invés de `ci=None`
