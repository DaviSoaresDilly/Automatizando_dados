...
def generate_occupacao_report(
    session: Session, clinica_id: int, dias_previstos: int = 30
) -> pd.DataFrame:
    ...
    ocupacao_media_historica = calcular_ocupacao_media(
        df_ocupacao, clinica.capacidade_leito
    )

    # Reindexar para garantir continuidade e definir a frequência
    all_dates = pd.date_range(
        start=ocupacao_media_historica.index.min(),
        end=ocupacao_media_historica.index.max(),
        freq="D",
    )
    ...
    relatorio_df = pd.concat(
        [historico_df.set_index("data"), previsao_df.set_index("data")], axis=1
    )
...
def generate_doenca_por_bairro_report(session: Session) -> pd.DataFrame:
    ...
    df_doenca_por_bairro = pd.DataFrame(
        result, columns=["Bairro", "Doença", "Quantidade"]
    )
...
def generate_age_gender_distribution_report(session: Session) -> pd.DataFrame:
    query = (
        session.query(Paciente, Atendimento, Doenca, Bairro)
    ...
    distribuicao_df = (
        df.groupby(["Faixa Etária", "Sexo", "Doenca"], observed=False)
        .size()
...
if __name__ == "__main__":
    ...
    with app.app_context():
        ...
        export_to_file(
            df_occupacao, f"occupacao_report_{clinica_id}_{timestamp}", "csv"
        )
        export_to_file(
            df_occupacao, f"occupacao_report_{clinica_id}_{timestamp}", "pdf"
        )
        export_to_file(
            df_distribuicao, f"distribuicao_faixa_etaria_sexo_{timestamp}", "csv"
        )
        export_to_file(
            df_distribuicao, f"distribuicao_faixa_etaria_sexo_{timestamp}", "pdf"
        )
