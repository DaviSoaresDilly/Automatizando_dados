...
with app.app_context():
    ...
    st.write(
        "Esta seção fornece uma visão geral da ocupação de leitos nos últimos seis meses, "
    ...
    try:
        ...
        if (
            "Ocupação Histórica" in df_occupacao.columns
            and "Ocupação Prevista" in df_occupacao.columns
        ):
            ...
            plot_ocupacao_historico_previsao(
                ...
                fig,
                ax,  # Passa a figura e o eixo para configuração
            )
            st.pyplot(fig)
        else:
    ...
    st.write(
        "Esta análise mostra a distribuição de doenças por faixa etária e gênero, permitindo identificar "
    ...
    st.write(
        "Este gráfico detalha a quantidade de casos de doenças por bairro, fornecendo insights sobre as áreas "
